...
public class FakturoidTodosProxy : FakturoidEntityProxy {
    ...
    public Task<IEnumerable<FakturoidTodo>> SelectAsync(DateTimeOffset? since = null) => this.GetAllPagedEntitiesAsync<FakturoidTodo>("todos.json", new { since });
    ...
    public async Task<IEnumerable<FakturoidTodo>> SelectAsync(int page, DateTimeOffset? since = null) => page < 1 ? throw new ArgumentOutOfRangeException(nameof(page), "Page must be greater than zero.") : await this.GetPagedEntitiesAsync<FakturoidTodo>("todos.json", page, new { since });
